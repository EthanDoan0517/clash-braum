...
rows=[]
for half in [.065,.085]:
 for offset in [-.08,-.06,-.04,-.02,0,.02,.04,.06,.08]:
  for advance in [0,.02,.04,.06,.08,.10,.12,.14]:
   center=palm+axis.cross(forward).normalized()*offset+forward*advance;ends=[center-axis*half,center+axis*half];anchors=[rail.find_nearest(p)[0] for p in ends]
...
rows.sort(key=lambda r:r['score']);(ROOT/'validation/local_handle_normal_search.json').write_text(json.dumps({'method':'144 bounded anatomical-normal running-E trials; reject triangle intersections against full Object004/Object009, rank unsigned palm/digit distances to bar. No multi-pose or visual acceptance implied.','collision_free':rows},indent=2));print(json.dumps(rows[:5],indent=2))
