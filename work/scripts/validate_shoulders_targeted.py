"""Cheap two-checkpoint shoulder diagnostics and protected-region comparison."""
from pathlib import Path
import bpy,json,hashlib,numpy as np
ROOT=Path(__file__).resolve().parents[2]
r=json.loads((ROOT/'validation/shoulder_refinement.json').read_text());regions=json.loads((ROOT/'validation/rig_regions.json').read_text())['objects']
allowed={n:set(v) for n,v in r['shoulder_changes']['changed_indices'].items()}
for c in r['torso_hardware_changes']:allowed.setdefault(c['object'],set()).update(next(v['indices'] for v in regions[c['object']] if v['id']==c['component']))
cases=[('braum_recall',f) for f in [60,65,70]]+[('braum_dance_loop',f) for f in [45,50,55]]+[('braum_spell4',f) for f in [3,15,26]]+[('braum_idle_01_loop',17),('braum_spell3_idle180',29),('braum_run_02',12)]
report={'revisions':{}};baseline={}
for label,file in [('before','clash_braum_deformation.blend'),('after','clash_braum_shoulders.blend')]:
 ...
 for clip,frame in cases:
  ...
  for n in allowed:
   ...
   row={'clip':clip,'frame':frame,'object':n,'max_elongation':float(d.max()),'p99_elongation':float(np.quantile(d,.99))}
   if n=='Object002':row['edge_1497_2321_length']=float(np.linalg.norm(p[1497]-p[2321]))
...
(ROOT/'validation/shoulder_targeted_validation.json').write_text(json.dumps(report,indent=2))
for a,b in zip(report['revisions']['before']['poses'],report['revisions']['after']['poses']):
 if a['frame'] in [65,50]:print(a['clip'],a['object'],round(a['max_elongation'],5),'->',round(b['max_elongation'],5),'edge',a.get('edge_1497_2321_length'),b.get('edge_1497_2321_length'))
