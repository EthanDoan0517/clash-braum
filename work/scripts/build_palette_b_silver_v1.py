"""Palette B silver v1 on the current integrated candidate; texture-only, no model export."""
import json, zipfile
import numpy as np
from PIL import Image
import build_lightning_trial as p
ROOT=p.ROOT;OUT=ROOT/'build/palette_b_silver_v1'
    ...
    for file in sorted((ROOT/'build/palette_a_inputs').glob('*.npy')):
        ...
        for tri in np.load(file):
            ...
            if shield:
                ...
                metal=np.clip(.68+.42*np.abs(normal[:,0])+.30*np.maximum(normal[:,2],0),.62,1.36)
                rgb=np.where(rim[:,None],np.array([154,164,178])*metal[:,None],np.array([39,46,63])*shade[:,None])
                if file.stem=='CCE_mesh_13.001':rgb=np.array([100,111,128])*shade[:,None]
            elif n=='Object002':
                rgb=np.array([65,72,133])*shade[:,None]
                ...
                sleeves=xyz[:,2]>1.30
                rgb=np.where(sleeves[:,None],np.array([209,203,192]),np.array([32,35,52]))*shade[:,None]
                # Police sleeve badge stays legible.
                badge=(old.max(1)-old.min(1)<25)&(lum>115)
                rgb[badge]=old[badge]
            elif n=='Object001':
                rgb=np.array([32,35,52])*shade[:,None]
            elif n=='Object006':
                rgb=np.array([49,47,66])*shade[:,None]
            elif n=='Object007':
                rgb=np.array([139,126,96])*shade[:,None]
            elif n=='hjhjh_2':
                rgb=np.array([39,40,61])*shade[:,None]
            elif n=='hjhjh_4':
...
def main():
    assert not (OUT/'Braum_Clash_Palette_B_Indigo_Silver_v1.fantome').exists(),'Preserve completed candidate.'
    ...
    info.update(Name='Clash Braum Palette B Indigo Silver v1 TRIAL',Version='0.4.2-palette-b-silver',Description='Indigo armor, warm gray sleeves, muted gold equipment accents, metallic silver shield edges over a dark lower panel and painted close-cropped hair. Current travelling-strikes VFX and English VO/audio preserved. Manual gameplay acceptance pending.')
    ...
    archive=OUT/'Braum_Clash_Palette_B_Indigo_Silver_v1.fantome'
    ...
    (ROOT/'validation/palette_b_silver_v1_build.json').write_text(json.dumps(report,indent=2))
    (ROOT/'validation/palette_b_silver_v1_build_logs.json').write_text(json.dumps(p.logs,indent=2))
